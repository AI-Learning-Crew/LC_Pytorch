...
def evaluate_retrieval_ranking(model, test_dataset, device, top_k: int = 5) -> pd.DataFrame:
    ...
    for i in range(len(image_file_paths)):
        ...
        img_id = os.path.normpath(image_file_paths[i]).split(os.sep)[4]
        
        row_data = {'Image_File': img_id}
        ...
        for rank, audio_idx in enumerate(top_k_indices):
            aud_id = os.path.normpath(audio_file_paths[audio_idx]).split(os.sep)[4]
            row_data[f'Rank_{rank+1}_Audio'] = aud_id
            row_data[f'Rank_{rank+1}_Score'] = similarity_matrix[i, audio_idx].item()
            
            # 정답 여부 확인
            is_match = (img_id == aud_id)
    ...
    for k in top_ks:
        ...
        for i in range(len(image_file_paths)):
            ...
            for audio_idx in top_k_indices:
                # print(f"Image: {os.path.basename(image_file_paths[i])}, Audio Indices: {os.path.basename(audio_file_paths[audio_idx])}")
